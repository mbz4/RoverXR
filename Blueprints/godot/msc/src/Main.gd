...
const PI_STREAM_HOST = "localhost"
...
#	print_debug("Attempting to connect... status = ", streamTCP.poll())
	if streamTCP.get_status() == 1:
		print_debug("Attempting to connect... status = ", streamTCP.poll())
	if streamTCP.get_status() == 2:
		print_debug("Connected to: " + streamTCP.get_connected_host() + " ", streamTCP.get_connected_port())
		pass
